...
CREATE INDEX idx_saved_lists_client ON saved_lists(client_user_id);
CREATE INDEX idx_invoices_client ON invoices(client_user_id, created_at DESC);
...
CREATE TRIGGER ensure_staff_profiles_user_role
BEFORE INSERT OR UPDATE ON staff_profiles
FOR EACH ROW
EXECUTE FUNCTION ensure_staff_user_role();

CREATE TRIGGER ensure_client_profiles_user_role
